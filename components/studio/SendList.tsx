...
export function SendList({ event, guests }: { event: InviteEvent; guests: Guest[] }) {
  ...
  return (
    <div className="mt-8 space-y-3">
      {guests.map((guest) => {
        const url = origin ? inviteUrl(origin, event.slug, guest.token) : "";
        const text = `${guest.name}, estás invitadx a ${event.content.title}. Abrí tu invitación: ${url}`;
        return (
          <div key={guest.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-ink/10 bg-paper px-4 py-3">
            ...
            <div className="flex gap-2">
              ...
              <a
                href={`mailto:${guest.email}?subject=${encodeURIComponent(event.content.title)}&body=${encodeURIComponent(text)}`}
